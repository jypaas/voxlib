/*
 * db_pool_async_pgsql_example.c - PostgreSQL 连接池 + 异步示例（模拟多"请求"并发）
 *
 * 关键点：
 * - 使用 vox_db_pool_t 避免单连接 busy
 * - 这里演示 VOX_DB_CALLBACK_LOOP：回调切回 loop，便于与上层事件驱动系统集成
 * - PostgreSQL 连接字符串格式：使用 libpq 原生格式（空格分隔）
 *   例如："host=127.0.0.1 port=5432 user=postgres password=xxx dbname=testdb"
 *
 * 编译要求：
 * - 需要启用 VOX_USE_PGSQL=ON
 * - 需要安装 libpq 开发库（PostgreSQL client library）
 *
 * 使用前准备：
 * 1. 确保 PostgreSQL 服务正在运行
 * 2. 创建测试数据库：CREATE DATABASE testdb;
 * 3. 修改下面的连接字符串（host, port, user, password, dbname）
 */
...
static void on_exec(vox_db_conn_t* conn, int status, int64_t affected_rows, void* user_data) {
    ...
    if (app->done >= app->total) {
        ...
        for (int i = 0; i < app->query_total; i++) {
            ...
            if (vox_db_pool_query_async(app->pool, "SELECT id, name FROM t WHERE id = $1 LIMIT 1;",
...
static void start_work(vox_loop_t* loop, void* user_data) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (vox_db_pool_exec_async(app->pool, "INSERT INTO t VALUES($1, $2);", req->params, 2, on_exec, req) != 0) {
...
int main(void) {
    ...
    /* PostgreSQL 连接字符串格式：使用 libpq 原生格式（空格分隔） */
    /* 请根据实际情况修改以下连接参数 */
    const char* pgsql_conninfo = "host=127.0.0.1 port=5433 user=testdb password=testdb dbname=testdb";
    
    VOX_LOG_INFO("main: creating database pool (PostgreSQL)...");
    /* 使用动态连接池：初始 8 个连接，最大 32 个连接 */
    vox_db_pool_t* pool = vox_db_pool_create_ex(loop, VOX_DB_DRIVER_PGSQL, pgsql_conninfo, 8, 100);
    if (!pool) {
        VOX_LOG_ERROR("main: failed to create PostgreSQL connection pool");
        VOX_LOG_ERROR("main: please check:");
        VOX_LOG_ERROR("  1. PostgreSQL server is running");
        VOX_LOG_ERROR("  2. Database 'testdb' exists (CREATE DATABASE testdb;)");
        VOX_LOG_ERROR("  3. Connection string is correct (host, port, user, password, dbname)");
